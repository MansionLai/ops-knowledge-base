...
| 腳本 | 用途 | 使用時機 |
| :--- | :--- | :--- |
| `windows-diag.ps1` | 全面收集系統狀態（Event Log、行程、網路、磁碟、安全性等） | 任何異常，或 user 當下仍有問題時 |

腳本輸出結構：
```
vm_diag_<ComputerName>_<timestamp>.zip
├── 01_system/        # 系統資訊、硬體規格、Hotfix 清單
├── 02_event_logs/    # System / Application / Security / Defender 等事件日誌
├── 03_processes/     # 行程、服務、排程任務、啟動項目
├── 04_network/       # 網路設定、連線、防火牆規則
├── 05_storage/       # 磁碟使用量、健康狀態、Shadow Copy
├── 06_security/      # 本地帳號、群組、登入 Session
├── 07_software/      # 安裝軟體清單、QEMU-GA 狀態
└── 08_crash_reports/ # WER 崩潰報告、Minidump 清單
```
...
### Step 1：複製腳本
將下方 **「腳本原始碼」** 區段的完整內容複製，貼到記事本，存成 `windows-diag.ps1`（放桌面即可）。

### Step 2：以系統管理員身分執行
按下 `Win + X`，選「**Windows PowerShell (系統管理員)**」，執行：

```powershell
# 預設收集（Event Log 範圍：過去 7 天）
powershell -ExecutionPolicy Bypass -File "$env:USERPROFILE\Desktop\windows-diag.ps1"
```

**若知道事發時間，指定時間段（推薦）：**
```powershell
powershell -ExecutionPolicy Bypass -File "$env:USERPROFILE\Desktop\windows-diag.ps1" -StartTime "2026-09-04 09:00" -EndTime "2026-09-04 13:00"
```

### Step 3：回傳壓縮檔
腳本完成後會顯示輸出路徑，請使用者將 `.zip` 傳回維運團隊：
```
...
| Event ID | Log | 意義 |
| :--- | :--- | :--- |
| **2004** | System | 虛擬記憶體嚴重不足，**直接列出當時前 3 名元兇** 🔴 |
| **41** | System | Kernel-Power：非預期關機／重啟 🔴 |
| **6008** | System | 上次關機是非預期的 🔴 |
| **1001** | Application | 藍屏 (BSOD) 🔴 |
| **1002** | Application | 程式無回應 (Hang) 🟠 |
| **1000** | Application | 程式崩潰 (Crash) 🟠 |
| **7034** | System | 服務意外終止 🟠 |
| **1074** | System | 主動重啟記錄 🟡 |

> [!IMPORTANT]
> **QEMU Guest Agent 狀態**：請在 `07_software/qemu_guest_agent.txt` 確認 QEMU-GA 是否正常執行。
> 若未運行，Prometheus 上的 `memory_available` 指標不可信，容易產生假性記憶體告警。

---

## 腳本原始碼

> 複製以下完整內容，存成 `windows-diag.ps1` 後執行。
...
1. **`01_system/system_overview.txt`** — 確認時區，對齊 Prometheus 告警時間
2. **`02_event_logs/system_events.txt`** — 搜尋 Event ID 2004 / 41 / 6008（記憶體耗盡 / 非預期關機）
3. **`02_event_logs/application_events.txt`** — 搜尋 Event ID 1002 / 1000（Hang / Crash）
4. **`03_processes/services.txt`** — 確認是否有服務在事發前後崩潰重啟
5. **`03_processes/scheduled_tasks.txt`** — 確認事發時間是否有排程任務觸發（備份、掃毒、更新）
6. **`08_crash_reports/wer_reports.txt`** — 確認是否有程式在事發時段產生 Crash Report
